...
/*   Updated: 2020/07/28 06:29:16 by phnguyen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_hud(game_t *g)
{
	...
	g->bonus->hud.img = mlx_xpm_file_to_image(g->mlx_ptr, "texture/crosshair.xpm",
		&g->bonus->hud.width, &g->bonus->hud.height);
	g->bonus->hud.data = (int*)mlx_get_data_addr(g->bonus->hud.img,
		&g->bonus->hud.bpp, &g->bonus->hud.sizeline, &g->bonus->hud.endian);
	g->bonus->map.img = mlx_new_image(g->mlx_ptr, g->p->res_w/6, g->p->res_h/6);
